...
app.get("/filter", (req, res) => {
  ...
  const filteredAct = jokes.filter((joke) => joke.jokeType == type);
  res.json(filteredAct);
});
...
app.put("/jokes/:id", (req, res) =>{
  ...
  const reJoke = {
    id:id,
    jokeTest: req.body.text,
    jokeType: req.body.type,
  };

  const searchIndex = jokes.findIndex((joke) => joke.id === id);

  jokes[searchIndex] = reJoke;
  res.json(reJoke);
})
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const exiJoke = jokes.find((joke) => joke.id === id);
  const reJoke ={
    id: id,
    jokeText: req.body.text || exiJoke.jokeText,
    jokeType: req.body.type || exiJoke.jokeType,
  };
  const searchIndex = jokes.findIndex((joke)=> joke.id === id);
  jokes[searchIndex] = reJoke;
  res.json(reJoke);
